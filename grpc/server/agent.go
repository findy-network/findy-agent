...
func processQuestion(question bus.AgentQuestion) (as *pb.AgentStatus) {
	...
	if question.IssuePropose != nil {
		...
		agentStatus.Notification.Question = &pb.Notification_IssuePropose{
			IssuePropose: &pb.Notification_IssueProposeMsg{
	...
	if question.ProofVerify != nil {
		...
		attrs := make([]*pb.Notification_ProofVerifyMsg_Attribute, 0, len(question.ProofVerify.Attrs))
		for _, attr := range question.Attrs {
			attrs = append(attrs, &pb.Notification_ProofVerifyMsg_Attribute{
				Value:     attr.Value,
				Name:      attr.Name,
		...
		agentStatus.Notification.Question = &pb.Notification_ProofVerify{
			ProofVerify: &pb.Notification_ProofVerifyMsg{
				Attributes: attrs,
			},
		}
	}
